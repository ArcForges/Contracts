...
public static class ContractShapeValidation
{
    ...
    /// <summary>Checks the declared wire/profile constraints of arcforges.operator.v1.OperatorCallContext.</summary>
    public static bool IsValid([global::System.Diagnostics.CodeAnalysis.NotNullWhen(true)] global::ArcForges.Contracts.CloudInternal.Operator.V1.OperatorCallContext? value) => Check(value, new ValidationContext());
    private static bool Check([global::System.Diagnostics.CodeAnalysis.NotNullWhen(true)] global::ArcForges.Contracts.CloudInternal.Operator.V1.OperatorCallContext? value, ValidationContext context)
    {
        if (value is null || !context.Enter(value)) return false;
        try
        {
        if ((int)value.ContextCase == 1)
        {
            if (!Check(value.CaseId, context)) return false;
        }
        if ((int)value.ContextCase == 2)
        {
