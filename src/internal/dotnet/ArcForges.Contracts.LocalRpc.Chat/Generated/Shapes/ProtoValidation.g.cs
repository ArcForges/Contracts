...
#nullable enable
using System.Text;
namespace ArcForges.Contracts.LocalRpc.Chat.Shapes;
...
public static class ContractShapeValidation
{
    ...
    /// <summary>Checks the declared presence, field bounds and shape rules of arcforges.local.chat.v1.ChatOperationsServiceOpenArtifactValue.</summary>
    public static bool IsValid(global::ArcForges.Contracts.LocalRpc.Chat.V1.ChatOperationsServiceOpenArtifactValue? value)
    {
        if (value is null) return false;
        if (value.Receipt is null) return false;
        if (value.Receipt is not null)
        {
            if (!IsValid(value.Receipt)) return false;
        }
        return true;
    }
