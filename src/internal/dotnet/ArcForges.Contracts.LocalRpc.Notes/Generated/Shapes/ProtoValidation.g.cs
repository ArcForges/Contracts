...
public static class ContractShapeValidation
{
    ...
    /// <summary>Checks the declared presence, field bounds and shape rules of arcforges.local.notes.v1.NotesOperationsServiceTrashDocumentValue.</summary>
    public static bool IsValid(global::ArcForges.Contracts.LocalRpc.Notes.V1.NotesOperationsServiceTrashDocumentValue? value)
    {
        if (value is null) return false;
        if (value.Version is null) return false;
        if (value.Version is not null)
        {
            if (!IsValid(value.Version)) return false;
        }
        return true;
    }
